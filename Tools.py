...
def get_hyper_SVM(x_d2_d,x_d3_d,x_d3_p,y_d2_d,y_d3_d,y_d3_p,seed=0,n_jobs = 15):            
        #res_c = [0.001,0.01,0.05,0.07,0.1,0.3,0.5,0.7,1,3,5,7, 10,30,50,70, 100,200]    
        res_c = [0.001,0.01,0.05,0.1,0.5,1,5,10,50, 100,200]
        #res_c = [0.001]
        #res_gamma = [1,0.7,0.5,0.3,0.1,0.07,0.05,0.03,0.01,0.007,0.005,0.003,0.001,0.0001]
        res_gamma = [1,0.5,0.1,0.05,0.01,0.005,0.001,0.0001]
        #res_gamma = [1]
        ...
        for c in res_c:
            for gamma in res_gamma:
                for kernel in res_kernel:
                    v.append((x_d2_d,y_d2_d,"d2_d",c,gamma,kernel))
                    v.append((x_d3_d,y_d3_d,"d3_d",c,gamma,kernel))
                    v.append((x_d3_p,y_d3_p,"d3_p",c,gamma,kernel))
        def calcu(v):
            x,y,tipe,c,gamma,kernel = v
            ...
            for train_index, test_index in skf.split(x, y):
                for train_index, test_index in skf.split(x, y):
                    ...
                    predi = svm.predict(x.iloc[test_index,:])
                    res.append(balanced_accuracy_score(y.iloc[test_index],predi))
            return np.mean(res)
        print("start")
        res = Parallel(n_jobs=n_jobs,verbose=7)(delayed(calcu)(p) for p in v)
        print("stop")
        tipe = []
        c = []
        gamma = []
        ...
        for i in range(len(v)):
            ...
            gamma.append(v[i][4])
            kernel.append(v[i][5])
        df = pd.DataFrame({"tipe":tipe,"c":c,"gamma":gamma,"kernel":kernel,"res":res})
        
        aux =df[df.tipe=="d2_d"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d2_d"]["c"] = aux.iloc[0].c
        comp["d2_d"]["gamma"] = aux.iloc[0].gamma
        comp["d2_d"]["kernel"] = aux.iloc[0].kernel
        
        aux =df[df.tipe=="d3_d"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d3_d"]["c"] = aux.iloc[0].c
        comp["d3_d"]["gamma"] = aux.iloc[0].gamma
        comp["d3_d"]["kernel"] = aux.iloc[0].kernel
        
        aux =df[df.tipe=="d3_p"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d3_p"]["c"] = aux.iloc[0].c
        comp["d3_p"]["gamma"] = aux.iloc[0].gamma
        comp["d3_p"]["kernel"] = aux.iloc[0].kernel
        return comp            

                
def get_hyper_RF(x_d2_d,x_d3_d,x_d3_p,y_d2_d,y_d3_d,y_d3_p,seed=0,n_jobs = 15):            
        res_min_samples_split = [2,3,4,5,7,9,10,12,16,18,20]
        max_features = ["log2"]
        v = []
        for mf in max_features:
            for min_samples_split in res_min_samples_split:
                v.append((x_d2_d,y_d2_d,"d2_d",mf,min_samples_split))
                v.append((x_d3_d,y_d3_d,"d3_d",mf,min_samples_split))
                v.append((x_d3_p,y_d3_p,"d3_p",mf,min_samples_split))
                
        def calcu(v):
            x,y,tipe,max_features,min_samples_split = v
            rf = RandomForestClassifier(n_estimators=4001,max_features=max_features,min_samples_split=min_samples_split,random_state=seed,class_weight="balanced_subsample")
            res = []
            skf = StratifiedKFold(n_splits=10,shuffle=True,random_state=seed)
            for train_index, test_index in skf.split(x, y):
                rf.fit(x.iloc[train_index,:], y.iloc[train_index])
                predi = rf.predict(x.iloc[test_index,:])
                res.append(balanced_accuracy_score(y.iloc[test_index],predi))
        ...
        df = pd.DataFrame({"tipe":tipe,"mf":mf,"mi":mi,"res":res})
        
        aux =df[df.tipe=="d2_d"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d2_d"]["mf"] = aux.iloc[0].mf
        comp["d2_d"]["mi"] = aux.iloc[0].mi
        
        aux =df[df.tipe=="d3_d"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d3_d"]["mf"] = aux.iloc[0].mf
        comp["d3_d"]["mi"] = aux.iloc[0].mi
        
        aux =df[df.tipe=="d3_p"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d3_p"]["mf"] = aux.iloc[0].mf
        comp["d3_p"]["mi"] = aux.iloc[0].mi
        
        return comp

def get_hyper_GB(x_d2_d,x_d3_d,x_d3_p,y_d2_d,y_d3_d,y_d3_p,seed=0,n_jobs = 15):            
        res_min_samples_split = [2,3,4,5,7,9,10,12,16,18,20]
        max_features = ["log2"]
        res_subsample = [1,0.8,0.6]
        v = []
        for mf in max_features:
            for mi in res_min_samples_split:
                for su in res_subsample:
                    v.append((x_d2_d,y_d2_d,"d2_d",mf,mi,su))
                    v.append((x_d3_d,y_d3_d,"d3_d",mf,mi,su))
                    v.append((x_d3_p,y_d3_p,"d3_p",mf,mi,su))
                
        def calcu(v):
            x,y,tipe,max_features,min_samples_split,subsample = v
            gb = GradientBoostingClassifier(n_estimators=4001,subsample=subsample,max_features=max_features,min_samples_split=min_samples_split,random_state=seed)
            res = []
            skf = StratifiedKFold(n_splits=10,shuffle=True,random_state=seed)
            for train_index, test_index in skf.split(x, y):
                gb.fit(x.iloc[train_index,:], y.iloc[train_index])
                predi = gb.predict(x.iloc[test_index,:])
                res.append(balanced_accuracy_score(y.iloc[test_index],predi))
        ...
        for i in range(len(v)):
            ...
            mi.append(v[i][4])
            su.append(v[i][5])
        df = pd.DataFrame({"tipe":tipe,"mf":mf,"mi":mi,"su":su,"res":res})
        
        aux =df[df.tipe=="d2_d"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d2_d"]["mf"] = aux.iloc[0].mf
        comp["d2_d"]["mi"] = aux.iloc[0].mi
        comp["d2_d"]["su"] = aux.iloc[0].su
        
        aux =df[df.tipe=="d3_d"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d3_d"]["mf"] = aux.iloc[0].mf
        comp["d3_d"]["mi"] = aux.iloc[0].mi
        comp["d3_d"]["su"] = aux.iloc[0].su
        
        aux =df[df.tipe=="d3_p"]
        aux.sort_values(by="res",ascending=False,inplace=True)
        comp["d3_p"]["mf"] = aux.iloc[0].mf
        comp["d3_p"]["mi"] = aux.iloc[0].mi
        comp["d3_p"]["su"] = aux.iloc[0].su
        return comp
